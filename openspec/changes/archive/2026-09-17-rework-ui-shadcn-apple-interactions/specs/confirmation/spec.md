...
Popconfirm SHALL 以触发按钮为锚点轻量展开，SHALL NOT 使用模态 AlertDialog 或 Dialog；展开与关闭 SHALL NOT 移动页面其他内容（稳定锚点位移 ≤1px）；打开确认框后焦点 SHALL 移入框内可操作元素，关闭后焦点 SHALL 回到触发按钮。确认框的视觉 SHALL 由 shadcn token 表达，其容器 SHALL 使用 `popover` 语义（`bg-popover`/`text-popover-foreground`/`border`），SHALL NOT 使用自建色值。
